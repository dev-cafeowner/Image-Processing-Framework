...
module ov7670_pclk_rx_tb;
    ...
    ov7670_pclk_rx #(.FIFO_DEPTH(DEPTH),.EXPECTED_WIDTH(W)) dut (
        .aclk(clk),.aresetn(resetn),.pclk(pclk),.cam_href(href),.cam_vsync(vsync),.cam_data(data),
    ...
    initial begin
        if (W==640) begin
            ...
            for(integer phase=0;phase<6;phase++) begin
                halfp=(phase<2) ? 41.666667 : ((phase<4) ? 20.833333 : 20.0);
                #(phase*1.37);
                reset_dut();
...
module ov7670_pclk_rx_vga_tb;
    ov7670_pclk_rx_tb #(.W(640),.H(480),.DEPTH(4096)) test();
endmodule
