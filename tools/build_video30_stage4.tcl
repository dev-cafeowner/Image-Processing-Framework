# Separate experimental returned-clock build; stage3 15fps artifacts are immutable.
set root [file dirname [file dirname [file normalize [info script]]]]
set out [file join $root Vivado qr_video30_stage4]
if {[file exists [file join $out qr_video30_stage4.xpr]]} {
    open_project [file join $out qr_video30_stage4.xpr]
} else {
    create_project qr_video30_stage4 $out -part xc7z020clg400-1
    ...
    update_ip_catalog
    foreach path {hardware/rtl/bridge/qr_rgb565_gray8_axis_tap.v hardware/rtl/video/video_preview_overlay.v hardware/rtl/camera/ov7670_camera_clock.v hardware/rtl/camera/ov7670_pclk_rx.v hardware/rtl/camera/ov7670_clean_sync_axis.v hardware/rtl/camera/ov7670_pclk_clean_clock.v} {
...
if {[llength [get_files -quiet */qr_ip1_bd.bd]] == 0} {
    ...
    set seed_dir [file join $root Vivado video30_stage4_seed vivado]
    file mkdir $seed_dir
...
if {[get_property TYPE [get_bd_cells ov7670_axis_0]] != "module"} {
    ...
    create_bd_cell -type module -reference ov7670_clean_sync_axis ov7670_axis_0
...
update_module_reference [get_ips -all *ov7670_axis_0*]
...
set camera_xci [get_files -quiet */qr_ip1_bd_ov7670_axis_0_*.xci]
if {[llength $camera_xci] != 1} {error "Expected one current camera module XCI"}
reset_target all $camera_xci
generate_target all $bd
puts [exec pwsh.exe -NoProfile -File [file join $root tools verify_video30_stage4.ps1]]
set wrapper [make_wrapper -files $bd -top]
add_files -norecurse $wrapper
set_property top qr_ip1_bd_wrapper [current_fileset]
if {[llength [get_files -quiet *zybo_z7_20_ov7670_stage4.xdc]] == 0} {
...
write_hw_platform -fixed -include_bit -force -file [file join $out qr_video30_stage4.xsa]
file copy -force [file join $out qr_video30_stage4.runs impl_1 qr_ip1_bd_wrapper.bit] [file join $out qr_video30_stage4.bit]
puts "VIDEO30_STAGE4_HARDWARE_PASS"
close_project
exit
