...
int main(void){
    u8 *cpu; reset();
    assert(frame_receiver_init(&dma,&buffers[0][0],64)==XST_SUCCESS && arms==1 && credits==1 && dispatches==1);
    capture_complete(0x31);frame_receiver_service();
    assert(arms==2 && credits==1); // rearmed BEFORE any candidate take or ACK
    assert(frame_receiver_take(0,&cpu)==XST_SUCCESS && cpu==buffers[0]);
    capture_complete(0x42);
    for(int k=0;k<100;k++){frame_receiver_service(); assert(arms==2 && !credits);}
    for(int i=0;i<64;i++) assert(cpu[i]==0x31);
    assert(frame_receiver_release()==XST_SUCCESS && arms==3 && credits==1);
    assert(frame_receiver_take(1,&cpu)==XST_SUCCESS && cpu==buffers[1]);
    capture_complete(0x53);frame_receiver_service();
    for(int i=0;i<64;i++)assert(cpu[i]==0x42);
    assert(frame_receiver_release()==XST_SUCCESS && arms==4);
    assert(frame_receiver_take(2,&cpu)==XST_SUCCESS && cpu==buffers[0] && cpu[0]==0x53);
    for(int fault=0;fault<5;fault++){
        reset(); assert(frame_receiver_init(&dma,&buffers[0][0],64)==XST_SUCCESS);
        capture_complete(7);
        ...
        assert(frame_receiver_failed() && arms==1 && credits==0);
        assert(frame_receiver_take(0,&cpu)==XST_FAILURE);
    }
    reset();regfile[0]=0;assert(frame_receiver_init(&dma,&buffers[0][0],64)==XST_FAILURE && arms==0);
