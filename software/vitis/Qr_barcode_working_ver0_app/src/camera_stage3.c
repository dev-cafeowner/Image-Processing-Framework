...
int camera_stage3_check(void)
{
    ...
    if (rd(0x0cU) != expected_version || rd(0x10U) != 0x43414d33U || !(rd(0x30U)&1U)) {
        runtime_log_printf("[FAIL] Stage3 CAM3/24MHz clock hardware missing; sensor speed unchanged\r\n");
        return XST_FAILURE;
    }
    return XST_SUCCESS;
}
...
int camera_stage3_sensor_check(ov7670_t *camera)
{
    ...
    if ((com10 & 0x32U) || (com11 & 0x80U) || (com14 & 0x18U) || (pll & 0xc0U)) {
        runtime_log_printf("[FAIL] Unexpected stage3 sensor timing registers\r\n");
        return XST_FAILURE;
    }
