...
static void stage6_pipeline_observe(void)
{
    u32 id, status;
...
#define STAGE6_VERBOSE 0
#define STAGE6_TRACE(...) do { if (STAGE6_VERBOSE) xil_printf(__VA_ARGS__); } while (0)
...
static void stage6_audit_candidates(u32 words, u32 frame, u32 count, const u8 *gray)
{
...
static u8 preview_gray[HDMI_DISPLAY_WIDTH * STAGE6_UI_PANEL_HEIGHT] QR_DMA_ALIGNED;
#else
...
static void stage6_preview_service(void);
...
static void stage6_report_window(XTime now, u32 cycle_us, u32 qr_us,
...
#endif
#if QR_PINGPONG
    qr_pingpong_report(Xil_In32(QR_HW_RUNTIME_BASEADDR + QR_CSR_DROP_COUNT));
#endif
...
#if QR_PINGPONG
        if (qr_pingpong_failed()) return XST_FAILURE;
#endif
...
#if QR_CAMERA_SOURCE_SYNC
    if (camera_stage3_check() != XST_SUCCESS) return XST_FAILURE;
#endif
    pid = 0U;
    ver = 0U;
...
#if QR_CAMERA_SOURCE_SYNC
            if (camera_stage3_sensor_check(camera) != XST_SUCCESS) return XST_FAILURE;
...
#define STAGE6_UI_HEIGHT             STAGE6_UI_PANEL_HEIGHT
...
)
{
    ...
    /* Preview is newer than the last analyzed snapshot: label its age. */
    {
        ...
        stage6_draw_text(image, 240U, 8U, age);
    }
...
static void stage6_preview_service(void)
{
    ...
    stage6_draw_ui(preview_gray, preview_qr_detected);
    ...
    video_bgr888_to_gray_rgb888(
        destination + STAGE6_UI_PANEL_HEIGHT * HDMI_DISPLAY_STRIDE,
        source + STAGE6_UI_PANEL_HEIGHT * VIDEO_VDMA_STRIDE,
        HDMI_DISPLAY_WIDTH * (HDMI_DISPLAY_HEIGHT - STAGE6_UI_PANEL_HEIGHT));
#else
    ...
    video_gray8_to_rgb888(destination, preview_gray,
                         HDMI_DISPLAY_WIDTH * STAGE6_UI_PANEL_HEIGHT);
    ui_done = qr_perf_now();
...
int stage6_qr_runtime_run(void)
{
...
#if QR_PINGPONG
    status = qr_pingpong_init(&image_dma, &image_buffers[0][0], QR_HW_IMAGE_BYTES);
#else
    status = stage6_arm_image_dma(&image_dma);
#endif
...
#if QR_CAMERA_SOURCE_SYNC
    if (camera_stage3_validate_input() != XST_SUCCESS) return XST_FAILURE;
#endif
    ...
    xil_printf("[BUILD] video30_stage4 pl_preview=1 source_sync=1 clean_pclk=1 camera_clkrc=%lu colorbars=%lu; VIDEO scan_sof includes repeats\r\n", (u32)QR_CAMERA_CLKRC, (u32)QR_CAMERA_COLORBARS);
#else
    xil_printf("[BUILD] video30_stage3 pl_preview=1 source_sync=1 camera_clkrc=%lu colorbars=%lu; VIDEO scan_sof includes repeats\r\n", (u32)QR_CAMERA_CLKRC, (u32)QR_CAMERA_COLORBARS);
#endif
#elif QR_PL_PREVIEW
    xil_printf("[BUILD] video30_stage2 pl_preview=1 camera_clkrc=%lu; SUMMARY preview=CPU submissions (0); VIDEO scan_sof includes repeats\r\n", (u32)QR_CAMERA_CLKRC);
#else
    xil_printf("[BUILD] video30_stage1 fused_preview=%lu per_frame_logs=%lu camera_clkrc=%lu neon=%d\r\n",
               (u32)QR_PREVIEW_FUSED, (u32)QR_PER_FRAME_LOGS, (u32)QR_CAMERA_CLKRC,
               video_pixel_ops_uses_neon());
#endif
    qr_decode_set_progress_callback(stage6_preview_service);
#if QR_PIPELINE_TRACE
    pipeline_csr=&csr;
    ...
    for (;;) {
        ...
        status = qr_pingpong_take(qr_csr_read(&csr, QR_CSR_IMAGE_FRAME_ID), &completed_image);
#else
        status = stage6_wait_dma(&image_dma, "Image");
#endif
...
#endif
#if QR_CANDIDATE_AUDIT
        stage6_audit_candidates(result_words, image_id, candidate_count, completed_image);
#endif
        ...
        if(frame_count>=5U && frame_count<=7U) {
            ...
            while(qr_perf_us(stall_start,qr_perf_now())<120000U) {
                stage6_preview_service(); usleep(100U);
            }
            ...
            if(before!=after || qr_pingpong_failed()) return XST_FAILURE;
        }
#endif
        perf_qr_start = qr_perf_now();
#if QR_CAMERA_COLORBARS
        camera_stage3_pattern_report(completed_image, frame_count);
#endif
...
#if QR_PINGPONG
        if (qr_pingpong_release() != XST_SUCCESS) return XST_FAILURE;
#endif
        ...
        if (STAGE6_VERBOSE) stage6_print_ui(detected);
        ...
        stage6_report_window(perf_end, qr_perf_us(perf_cycle, perf_end),
